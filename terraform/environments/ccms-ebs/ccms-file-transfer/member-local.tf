locals {
  ...
  nonprod_sans = [
    format("%s-sftp-bc.%s-%s.modernisation-platform.service.justice.gov.uk", local.application_name, var.networking[0].business-unit, local.environment)
  ]

  prod_sans = [
    format("%s-sftp-bc.%s", local.application_name, local.prod_domain)
  ]
