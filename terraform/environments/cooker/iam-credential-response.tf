...
# Slack webhook stored as a SecureString in SSM
resource "aws_ssm_parameter" "slack_webhook_url" {
  name  = "/iam-credential-responder/slack-webhook-url"
  type  = "SecureString"
...
resource "aws_lambda_function" "credential_responder" {
  ...
  description      = "Disables exposed IAM keys, quarantines users, and raises alerts"
  ...
  environment {
    variables = {
      ...
      SLACK_WEBHOOK_SSM_PATH   = aws_ssm_parameter.slack_webhook_url.name
    }
  }
}
...
data "archive_file" "credential_responder" {
  type        = "zip"
  source_file = "${path.module}/lambda/credential_responder/handler.py"
  output_path = "${path.module}/lambda/credential_responder/handler.zip"
}

variable "slack_webhook_url" {
  type      = string
  sensitive = true
  description = "Slack webhook URL for #security-alerts notifications"
}
